...
const CardDetail: React.FC = () => {
  ...
  const handleAddToCart = () => {
    if (card && quantity > 0) {
      ...
      setSnackbarMessage(`${quantity} x ${card.name} adicionado(s) ao carrinho!`);
        ...
        <Grid item xs={12} md={6}>
          <Paper elevation={3} sx={{ p: 2, textAlign: 'center' }}>
            <CardMedia
              ...
              image={card.image || `https://via.placeholder.com/400x560/333/fff?text=${card.name}`}
              alt={card.name}
            />
          </Paper>
        </Grid>
          ...
          <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Typography 
                  variant="body1" 
                  color={card.stock > 10 ? 'success.main' : card.stock > 0 ? 'warning.main' : 'error.main'}
                >
                  {card.stock > 0 ? `${card.stock} em estoque` : 'Fora de estoque'}
                </Typography>
            ...
            <Paper sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Typography fontWeight="bold">R$ {card.price.toFixed(2)}</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography color="text.secondary">Estoque:</Typography>
                  <Typography fontWeight="bold">{card.stock} unidades</Typography>
                </Box>
              </Box>
            </Paper>
          </Grid>
