...
const Assistente: React.FC = () => {
  ...
  const simulateAIResponse = (userMessage: string): Message => {
    ...
    if (lowerMessage.includes('rara') || lowerMessage.includes('legendary')) {
      ...
      responseText = `Aqui estão algumas das cartas mais raras da nossa coleção! As cartas Legendary são as mais valiosas e difíceis de encontrar. ${rareCards.length > 0 ? 'Confira estas opções abaixo:' : 'No momento não temos cartas raras em estoque.'}`;
    } else if (lowerMessage.includes('fogo') || lowerMessage.includes('fire')) {
      const fireCards = cards.filter(card => 
        card.type.toLowerCase().includes('fire') || card.type.toLowerCase().includes('fogo')
      ).slice(0, 3);
    ...
    } else if (lowerMessage.includes('cara') || lowerMessage.includes('preço') || lowerMessage.includes('expensive')) {
    ...
    } else if (lowerMessage.includes('iniciante') || lowerMessage.includes('beginner') || lowerMessage.includes('começ')) {
      const beginnerCards = cards.filter(card => 
    ...
    } else if (lowerMessage.includes('promoção') || lowerMessage.includes('desconto') || lowerMessage.includes('oferta')) {
      const promotionCards = cards.filter(card => card.price < 50).slice(0, 3);
    ...
    } else if (lowerMessage.includes('cupom') || lowerMessage.includes('coupon')) {
    ...
    } else if (lowerMessage.includes('tipo') || lowerMessage.includes('element')) {
      const types = [...new Set(cards.map(card => card.type))];
      responseText = `Temos cartas dos seguintes tipos: ${types.join(', ')}. Cada tipo tem suas próprias características e estratégias únicas. Sobre qual tipo você gostaria de saber mais?`;
    } else if (lowerMessage.includes('estoque') || lowerMessage.includes('stock')) {
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Box>
            ...
            <IconButton onClick={() => setIsMinimized(!isMinimized)} title="Minimizar">
                        ...
                        {message.sender === 'assistant' && (
                          <>
                            <IconButton
                              ...
                              color={message.helpful === true ? 'success' : 'default'}
                              sx={{ opacity: 0.7 }}
                            >
                            ...
                            <IconButton
                              ...
                              color={message.helpful === false ? 'error' : 'default'}
                              sx={{ opacity: 0.7 }}
                            >
